...
typedef enum _MTSidePanelStyle {
    MTSidePanelSingleActive = 0,
    MTSidePanelMultipleActive
} MTSidePanelStyle;

typedef enum _MTSidePanelState {
    MTSidePanelCenterVisible = 1,
    MTSidePanelLeftVisible,
    MTSidePanelRightVisible
} MTSidePanelState;
@class MTSideVC;
@protocol MTSideVCDelegate <NSObject>

-(void)undoFocusMenuItem;

@end
@interface MTSideVC : UIViewController<UIGestureRecognizerDelegate>
@property(nonatomic,assign) IBOutlet id<MTSideVCDelegate> delegateVC;
#pragma mark - Usage
...
// style
@property (nonatomic) MTSidePanelStyle style; // default is MTSidePanelSingleActive
...
@property (nonatomic, readonly) MTSidePanelState state;
