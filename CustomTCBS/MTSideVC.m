...
- (void)_baseInit {
    self.style = MTSidePanelSingleActive;
...
- (void)viewDidLoad {
    ...
    self.state = MTSidePanelCenterVisible;
...
- (void)willAnimateRotationToInterfaceOrientation:(__unused UIInterfaceOrientation)toInterfaceOrientation duration:(NSTimeInterval)duration {
    ...
    if (self.centerPanelHidden) {
        ...
        frame.origin.x = self.state == MTSidePanelLeftVisible ? self.centerPanelContainer.frame.size.width : -self.centerPanelContainer.frame.size.width;
...
- (void)setState:(MTSidePanelState)state {
    if (state != _state) {
        _state = state;
        switch (_state) {
            case MTSidePanelCenterVisible: {
...
- (void)setStyle:(MTSidePanelStyle)style {
    if (style != _style) {
        _style = style;
...
- (void)_layoutSideContainers:(BOOL)animate duration:(NSTimeInterval)duration {
    ...
    if (self.style == MTSidePanelMultipleActive) {
...
- (void)setCenterPanel:(UIViewController *)centerPanel {
    ...
    if (centerPanel != _centerPanel) {
        ...
        [_centerPanel addObserver:self forKeyPath:@"viewControllers" options:0 context:&mt_Context];
        [_centerPanel addObserver:self forKeyPath:@"view" options:NSKeyValueObservingOptionInitial context:&mt_Context];
        if (self.state == MTSidePanelCenterVisible) {
            self.visiblePanel = _centerPanel;
        }
    }
    if (self.isViewLoaded && self.state == MTSidePanelCenterVisible) {
        ...
        MTSidePanelState previousState = self.state;
        self.state = MTSidePanelCenterVisible;
        [UIView animateWithDuration:0.2f animations:^{
            if (self.bounceOnCenterPanelChange) {
                // first move the centerPanel offscreen
                CGFloat x = (previousState == MTSidePanelLeftVisible) ? self.view.bounds.size.width : -self.view.bounds.size.width;
...
- (void)_swapCenter:(UIViewController *)previous previousState:(MTSidePanelState)previousState with:(UIViewController *)next {
    if (previous != next) {
...
- (void)setLeftPanel:(UIViewController *)leftPanel {
    if (leftPanel != _leftPanel) {
        ...
        if (self.state == MTSidePanelLeftVisible) {
...
- (void)setRightPanel:(UIViewController *)rightPanel {
    if (rightPanel != _rightPanel) {
        ...
        if (self.state == MTSidePanelRightVisible) {
...
- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldReceiveTouch:(UITouch *)touch
{
    ...
    CGPoint touchPoint = [touch locationInView:self.view];
        //check điều kiện vuốt từ mép để show menu
        if (touchPoint.x>30) {
            return NO;
        }
        return YES;
...
- (void)_handlePan:(UIGestureRecognizer *)sender {
    ...
    if ([sender isKindOfClass:[UIPanGestureRecognizer class]]) {
        ...
        self.centerPanelContainer.frame = frame;
        NSLog(@"x: %f ~~~~~~~ y: %f",self.centerPanelContainer.frame.origin.x,self.centerPanelContainer.frame.origin.y);
        ...
        if (self.style == MTSidePanelMultipleActive || self.pushesSidePanels) {
...
//                UIViewController* vc = [self viewControllerOfView:self.centerPanelContainer];
//            
//                if (vc) {
...
- (CGFloat)_correctMovement:(CGFloat)movement {
    ...
    } else if (self.state == MTSidePanelRightVisible && !self.allowRightOverpan) {
        if (position < -self.rightVisibleWidth) {
            return 0.0f;
        } else if ((self.style == MTSidePanelMultipleActive || self.pushesSidePanels) && position > 0.0f) {
    ...
    } else if (self.state == MTSidePanelLeftVisible  && !self.allowLeftOverpan) {
...
- (void)_loadCenterPanelWithPreviousState:(MTSidePanelState)previousState {
    [self _placeButtonForLeftPanel];
...
- (void)_showLeftPanel:(BOOL)animated bounce:(BOOL)shouldBounce {
    self.state = MTSidePanelLeftVisible;
    [self _loadLeftPanel];
    ...
    if (self.style == MTSidePanelSingleActive) {
        self.tapView = [[UIView alloc] init];
    }
    [self _toggleScrollsToTopForCenter:NO left:YES right:NO];
    if([_delegateVC respondsToSelector:@selector(undoFocusMenuItem)])
    {
        [_delegateVC undoFocusMenuItem];
    }
}

- (void)_showRightPanel:(BOOL)animated bounce:(BOOL)shouldBounce {
    self.state = MTSidePanelRightVisible;
    [self _loadRightPanel];
...
- (void)toggleLeftPanel:(__unused id)sender {
    if (self.state == MTSidePanelLeftVisible) {
        [self _showCenterPanel:YES bounce:NO];
    } else if (self.state == MTSidePanelCenterVisible) {
        [self _showLeftPanel:YES bounce:NO];
    }
}
...
- (void)setCenterPanelHidden:(BOOL)centerPanelHidden animated:(BOOL)animated duration:(NSTimeInterval) duration {
    if (centerPanelHidden != _centerPanelHidden && self.state != MTSidePanelCenterVisible) {
